function clean_img = CP_Cluster_Parllel(noisy_img,CPparams)
% CP decomposition with parallel computation technique on CPU
...
parfor k = 1:nclusters
    ...
    clean_blocks_cell{k} = double(CP);
end

for k = 1 : nclusters
    clean_blocks(:,:,:,idx==k) = clean_blocks_cell{k};
end
