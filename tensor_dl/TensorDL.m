...
if ~isfield(params, 'overlap_sz')
    bparams.overlap_sz = fix(bparams.block_sz/2);% 往下取整
...
info = bparams;
noisy_blocks = ExtractBlocks( noisy_img, bparams );% 抽取块
...
idx = fkmeans(X, nclusters, fkmeans_opt); % 先进行聚类,返回每一个块索引对应的类别record,1*16129

% denoise each group by tensor factorization 利用分解进行每一组的去噪
