function [D,Gamma,err,gerr] = kcpd20140705(params,varargin)
...
% % D = odct3dict(tensorblocksize,[8 8 4]);
% dictdimshow = [16 16 4];% [8 8 4];
% temp1 =odctdict(tensorblocksize(1),dictdimshow(1));
% temp2 =odctdict(tensorblocksize(2),dictdimshow(2));
% temp3 =odctdict(tensorblocksize(3),dictdimshow(3));
% 
% D = zeros([tensorblocksize prod(dictdimshow)]);
% indxijk = 0;
% 
...
%             D(:,:,:,indxijk) = atomtemp/norm(atomtemp(:));
%         end
%     end
% end

D = params.Dinit;   % 初始化字典

% if (isfield(params,'initdict'))
%   if (any(size(params.initdict)==1) && all(iswhole(params.initdict(:))))
% %     D = data(:,params.initdict(1:dictsize));
%     eval(['D = data(',repmat(':,',1,tensorblocksize),'params.initdict(1:dictsize));'])
%   else
...
%     eval(['D = params.initdict(',repmat(':,',1,tensorblocksize),'1:dictsize);'])
%   end
% else
%   data_ids = find(block_squared(data,tensorblocksize) > 1e-6);   % ensure no zero data elements are chosen
%   perm = randperm(length(data_ids));
%   D = data(:,data_ids(perm(1:dictsize)));
%   eval(['D = data(',repmat(':,',1,tensorblocksize),'data_ids(perm(1:dictsize)));'])
% end
...
maxsignals = 7000;   % 限制最大样本数，当样本多余1万时cp_als会很慢，多余2万时几乎停止
...
Patom = tensor(Patom);
atom = Patom.data;
if length(size(P))== 3 && length(tensorblocksize) == 3
    gamma_j = P.lambda;
elseif length(size(P))== 4 && length(tensorblocksize) == 3
%     gamma_j = reshape(P.lambda*P.U{4},1, length(data_indices));
    atomvect = vect_tensor(atom,tensorblocksize, 'tensor2vect');
    Xres = Xvect(:,data_indices)- Dvect*Gamma(:,data_indices) + Dj*gamma_j; % 计算所有用到该原子的信号未用该原子表达时的残差
    gamma_j = sum(Xres.*repmat(atomvect,1,length(data_indices)));% 内积
else
...
%     unused_sigs = unused_sigs([1:i-1,i+1:end]);        
    ...
    dtemp = X(:,unused_sigs(i)) - D*Gamma(:,unused_sigs(i)); % 找到未使用的信号的最大残差信号    
    dtemptensor = vect_tensor(dtemp,tensorblocksize, 'vect2tensor');    % 张量化并得到rank-1分解结果
...
% 重新编写该段代码，适用于CPD方法，计算每个小数据块的元素平方和
